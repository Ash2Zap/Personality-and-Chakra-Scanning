# app.py — Personality + Chakra Scan (Soulful Academy, Cloud-safe)
# - No JS; radio-as-dots with CSS
# - Built-in fonts in PDF (ReportLab-safe)
# - Labels fixed in two columns
# - MyAuraBliss crystals + 5-page PDF
...
.header-band h1 {{
  margin:0; font-family:'Playfair Display',serif; letter-spacing:.6px;
...
.left-label, .right-label {{
  font-size:14px; color:#5a4a72; font-family:'Poppins',sans-serif; line-height:1.35;
}}
...
.stRadio label {{
  ...
  color:transparent !important;  /* hide numbers */
}}
.stRadio label:hover {{ transform:scale(1.08); }}
/* fill center when selected (multiple wrappers supported) */
.stRadio input[type="radio"]:checked + div > span::after,
.stRadio input[type="radio"]:checked + span::after {{
  content:""; width:10px; height:10px; background:var(--purple); border-radius:50%;
  position:absolute;
}}
...
.stButton > button {{
  ...
  color:{PRIMARY_PURPLE}; border:none; font-weight:700; border-radius:999px;
  padding:10px 20px; box-shadow:0 10px 24px rgba(255,184,71,.35);
...
logo_file = st.file_uploader("Upload your Soulful Academy logo (optional, shows in PDF)", type=["png","jpg","jpeg"])
...
def chakra_remedy_block(status: str, chakra: str) -> str:
    base = {
        ...
        "Crown":"Morning silence, seva, read wisdom text.",
    }.get(chakra, "")
...
if st.button("📊 Calculate Results", type="primary"):
    ...
    for ch, v in chakra_avg.items():
        status = chakra_status(v)
        color  = CHAKRA_COLORS.get(ch, "#666")
        chip   = f"<span style='background:{color};color:#fff;padding:4px 10px;border-radius:999px;font-size:12px;margin-right:6px'>{ch}</span>"
        st.markdown(chip + f" **{status}** · Avg {v:.1f} — {chakra_remedy_block(status, ch)}", unsafe_allow_html=True)

    # PDF (no custom font names—built-in only)
    def build_pdf(traits: Dict[str,float], chakras: Dict[str,float], logo_bytes: Optional[bytes]) -> bytes:
        ...
        def title(txt):  # use default font (Helvetica) to avoid mapping errors
            return Paragraph(f"<para align='center'><b>{txt}</b></para>", styles["Title"])

        story.append(title("Personality Profile (Big Five style)"))
        story.append(Spacer(1, 12))
        tdata = [["Trait","Score (-3..+3)","Summary"]]
        for t,s in traits.items():
            tdata.append([t, f"{s:.2f}", summarize_trait(t,s)])
        tbl = Table(tdata, colWidths=[3*cm, 4*cm, 9*cm])
        ...
        chakra_table("Chakra Scan — Part 1", items[:3])
        chakra_table("Chakra Scan — Part 2", items[3:5])
